...
def get_args():
    ...
    parser.add_argument('--env', default='PongNoFrameskip-v4', type=str, help='gym environment') #BreakoutDeterministic-v4
...
def train(shared_model, shared_optimizer, rank, args, info):
    ...
    model = NNPolicy(channels=4, memsize=args.hidden, num_actions=args.num_actions) # a local/unshared model
    ...
    while info['frames'][0] <= 30e6 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):
            episode_length += 1
            value, logit = model(inputs=state.view(1,4,80,80))
            ...
            if rank == 0 and time.time() - last_disp_time > 60: # print info ~ every minute
                ...
                printlog(args, 'time {}, episodes {:.0f}, frames {:.1f} M, mean_epr {:.2f}, run_loss {:.2f}'
