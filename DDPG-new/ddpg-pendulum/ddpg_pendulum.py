...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_size, action_range):
...
class DDPG(object):
    def __init__(self, options):
        ...
        self.obs_dim = options.get('obs_dim')
        ...
        # actor model
        self.actor = Actor(self.obs_dim, self.action_size, self.action_range)
        self.actor_target = Actor(self.obs_dim, self.action_size, self.action_range)

        # critic model
        self.critic = Critic(self.obs_dim, self.action_size, self.action_range)
        self.critic_target = Critic(self.obs_dim, self.action_size, self.action_range)
...
def main(args):
    ...
    for e in range(args.episode):
        ...
        state = env.reset()
        state = np.reshape(state, [1, agent.obs_dim])
        while not done:
            step += 1
            total_step += 1
            if args.render:
            ...
            next_state = np.reshape(next_state, [1, agent.obs_dim])
            ...
            if done:
                ...
                writer.add_scalar('train/reward', score, total_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env-name', default='Pendulum-v0', type=str, help='open-ai gym environment')
    ...
    parser.add_argument('--save_per_episode', type=int, default=1000)
    parser.add_argument('--try_discrete_action', action='store_true')
