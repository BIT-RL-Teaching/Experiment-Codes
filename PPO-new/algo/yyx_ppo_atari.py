...
class PPO_Atari():
    def __init__(self,
        ...
        # for Swimmer
        if len(self.obs_space.shape) == 1 and self.action_type == 'continuous':
           raise NotImplementedError
        # for atari
        elif len(self.obs_space.shape) == 3 and self.action_type == 'discrete':
            self.actor_critic = Atari_Actor_Critic(self.obs_space.shape[0], self.act_space.n).to(self.device)
        # for CarRacing
        elif len(self.obs_space.shape) == 3 and self.action_type == 'continuous':
            self.actor_critic = CarRacing_Actor_Critic(self.obs_space.shape[0], self.act_space.shape[0]).to(self.device)
        else:  # 只剩最后一种了，最简单的cartpole
            raise NotImplementedError
    ...
    def _rollout_init(self):
        ...
        if self.action_type == 'discrete':  # atari
        ...
        elif self.action_type == 'continuous':  # CarRacing
            self.actions = torch.zeros(self.T_horizon, self.n_rollout_threads, self.act_space.shape[0]).to(self.device)
            self.logprob_a = torch.zeros(self.T_horizon, self.n_rollout_threads, self.act_space.shape[0]).to(self.device)
    ...
    def rollout_insert(self, obs, a, logprob_a, value_pred, r, done):
        ...
        self.step = (self.step + 1) % self.T_horizon

    def compute_return(self):
        with torch.no_grad():
            # like i神的main.py，line211
    ...
    def get_act_entropy(self, obs):
        assert self.action_type == 'discrete'
        _, actor_actprobs = self.actor_critic(obs)
        c = Categorical(actor_actprobs)
        entropy = c.entropy().item()
        return entropy
    ...
    def PPO_update_actor_critic(self, s, a, logprob_a, adv, value_past, td_target, actor_critic, optimizer, optim_iter_num):
        ...
        for j in range(optim_iter_num):
            ...
            if self.args.debug_use_ppo2_value_loss_clip:
