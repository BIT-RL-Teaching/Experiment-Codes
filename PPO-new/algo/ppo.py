...
import math
from algo.net import Mujoco_Actor, Mujoco_Critic
...
class PPO():
    def __init__(self,
        ...
        self.action_bound = (self.act_space.low[0], self.act_space.high[0])
    ...
    def save(self, timestep, is_evaluate, is_newbest=False):
        ...
        if is_newbest:  #
            ...
            if not os.path.exists(save_dir): os.makedirs(save_dir)
            # torch.save(self.critic[i].state_dict(), save_dir + f"/ppo_critic_ts{timestep}.pth")
            torch.save(self.actor.state_dict(), save_dir + f"/ppo_actor_ts{timestep}.pth")
    ...
    def select_action_for_vec(self, obs, mode):
        ...
        # 连续动作空间
            ...
            if mode == 'run':
                ...
                a = dist.sample()
                a = torch.clamp(a, self.action_bound[0], self.action_bound[1])
